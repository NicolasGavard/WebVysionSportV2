<?php
  $page_title                   = "Mes diètes en cours";
  $page_name                    = "Nom template de diète";
  $page_assigned_for            = "Prénom Nom";
  $page_list_assigned_for_one   = "Elève";
  $page_list_assigned_for_plur  = "Elèves";
  $page_add_date_begin          = "Sélectionnez une date de début";
  $page_date_begin              = "Date de début";
  $page_date_duration           = "Durée";
  $page_date_tags               = "Tags";
  $page_advancement             = "Avancement";
  $page_action                  = "Actions";
  $page_filter_title            = "Filtrer mes diètes en cours";
  $page_add_title               = "Ajouter une diète";
  $page_update_title            = "Modifier la diète";
  $page_delete_title            = "Supprimer la diète";
  $page_restore_title           = "Restaurer la diète";
  
  $page_meal_title              = "Voir les repas de la diète";
  $page_days                    = "Jours";
  $page_meal_type               = "Repas";
  $page_meal_foods              = "Composition";
  $page_meal_calorie            = "Calorie";
  $page_meal_proetin            = "Protéine";
  $page_meal_glucide            = "Glucide";
  $page_meal_lipid              = "Lipide";
  $page_meal_quantity           = "Quantité";

  $errorData_txt_template       = "Le modèle de la template de diète est manquant !!";
  $errorData_txt_student        = "Aucun élèves assignés sur la diète !!";
  $errorData_txt_dateStart      = "La date de début de la diète est manquante !!";