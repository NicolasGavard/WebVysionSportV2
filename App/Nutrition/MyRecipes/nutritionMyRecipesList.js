    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.ConfirmSave) {
          ...
          setTimeout(function () {
            window.location.href = "./nutritionMyRecipesList.php";
          }, 800);
        } else {
...
function ListMyRecipe(elemState) {
  ...
  $.map(dataTableData, function (val, key) {
    if (val.elemState == elemState) {
      ...
      const line =
        "<tr>" +
        '  <td style="padding:1rem;">&nbsp;&nbsp;<img style="max-height:100px; max-width:100px;" src="' +
        val.linkToPicture +
        '"/></td>' +
        "  <td>" +
        val.name +
        ...
        '        <a class="dropdown-item"                                                                               onclick="ViewMyRecipeFood(\'' +
        val.id +
        "', '" +
        val.name +
        '\');" href="#"><i class="dw dw-harvest"></i> Aliments</a>' +
        '        <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddMyRecipe"      onclick="ViewMyRecipe(\'' +
        val.id +
        "', '" +
        val.name +
        ...
        actionBtnDelete +
        '"  data-toggle="modal" data-target="#modalDel"              onclick="DelMyRecipe(\'' +
        val.id +
        "', '" +
        val.name +
        ...
        '        <a class="dropdown-item ' +
        actionBtnRestore +
        '" data-toggle="modal" data-target="#modalRest"             onclick="RestMyRecipe(\'' +
        val.id +
        "', '" +
        val.name +
