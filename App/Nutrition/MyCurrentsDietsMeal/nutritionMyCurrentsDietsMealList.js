...
$(function () {
  ...
  $(".btn-warning").on("click", function () {
    ...
    myCurrentDietMealsTable.clear().draw();
  ...
  $(".btnAddMyCurrentDietMeal").on("click", function () {
    ...
    $(".AddMyCurrentsDietsFormIdUserCoatch").val(
    ...
    if (template != 0 && student != 0 && dateStart != "") {
      $.ajax({
        ...
        success: function (data) {
          ...
          setTimeout(function () {
            window.location.href = "./nutritionMyCurrentsDietsList.php";
          }, 800);
        },
        error: function (data) {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        $.map(val.foods, function (valFoods, keyFoods) {
          ...
          listFoodsNutritionals +=
            '  <div class="row">' +
            '    <div class="col-md-3 col-sm-3"><span class="micon dw dw-flash"></span> ' +
            valFoods.calorie +
            "</div>" +
            '    <div class="col-md-3 col-sm-3"><span class="micon dw dw-orange"></span> ' +
            valFoods.proetin +
            "</div>" +
            '    <div class="col-md-3 col-sm-3"><span class="micon dw dw-chip"></span> ' +
            valFoods.glucide +
            "</div>" +
            '    <div class="col-md-3 col-sm-3"><span class="micon dw dw-flame"></span> ' +
            valFoods.lipid +
            "</div>" +
            "  </div>";
          listFoodsWeight +=
            '  <div class="row">' +
            '    <div class="col-md-3 col-sm-3"><span class="micon dw dw-shopping-bag"></span> ' +
            valFoods.weight +
            " " +
            valFoods.abbrWeightType +
            "</div>" +
            "  </div>";
        });
        ...
        let line =
          ...
          '        <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddMyCurrentDietMeal"  onclick="ViewMyCurrentDietMeal(\'' +
          val.id +
          '\');"                                                  href="#"><i class="dw dw-edit2"></i> Voir</a>' +
          '        <a class="dropdown-item ' +
          actionBtnDelete +
          '"  data-toggle="modal" data-target="#modalDel"                   onclick="DelMyCurrentDietMeal(\'' +
          val.id +
          "', '" +
          val.nameMealType +
          "-" +
          ...
          '        <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"                  onclick="RestMyCurrentDietMeal(\'' +
          val.id +
          "', '" +
          val.nameMealType +
          "-" +
...
function ViewMyCurrentDietMeal(id) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".AddMyCurrentsDietsFormIdUserCoatch").val(
        data.ViewMyCurrentDietMeal.idUserCoach
      );

      $(
        '.listMyTemplates option[value="' +
          data.ViewMyCurrentDietMeal.idDietTemplate +
          '"]'
      ).prop("selected", true);
      nameTemplate = $(
        '.listMyTemplates option[value="' +
      ...
      $(
        '.listStudents option[value="' +
          data.ViewMyCurrentDietMeal.idUserStudent +
          '"]'
      ).prop("selected", true);
      nameStudent = $(
        '.listStudents option[value="' +
      ...
      $(".AddMyCurrentDietMealFormTimestamp").val(
        data.ViewMyCurrentDietMeal.timestamp
      );
      $(".AddMyCurrentDietMealFormStatut").val(
        data.ViewMyCurrentDietMeal.elemState
      );
    },
    error: function (data) {
      console.log(data);
