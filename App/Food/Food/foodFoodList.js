  ...
  $.map(dataTableData, function (val, key) {
    if (val.elemState == elemState) {
      ...
      const line =
        ...
        val.name +
        '</br><span style="font-size:13px; color:#90a4ae;">' +
        val.code +
        "</span></td>" +
        ' <td><img style="max-height:40px; max-width:40px;" src="' +
        val.pictureBrand +
        '"/></td>' +
        ' <td><img style="max-height:40px; max-width:40px;" src="' +
        val.pictureScoreNutri +
        '"/></td>' +
        ' <td><img style="max-height:40px; max-width:40px;" src="' +
        val.pictureScoreNova +
        '"/></td>' +
        ' <td><img style="max-height:40px; max-width:40px;" src="' +
        val.pictureScoreEco +
        '"/></td>' +
        ...
        '       <a class="dropdown-item"                                                                          onclick="ViewDetailFood(\'' +
        val.id +
        '\');"             href="#"><i class="dw dw-analytics-5"></i> Détail</a>' +
        '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddFood"     onclick="ViewFood(\'' +
        val.id +
        "', '" +
        val.name +
        ...
        actionBtnDelete +
        '"  data-toggle="modal" data-target="#modalDel"         onclick="DelFood(\'' +
        val.id +
        "', '" +
        val.name +
        ...
        '       <a class="dropdown-item ' +
        actionBtnRestore +
        '" data-toggle="modal" data-target="#modalRest"        onclick="RestFood(\'' +
        val.id +
        "', '" +
        val.name +
...
function ViewFood(id, name) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#select2-listNutriScores-container").html(
        data.ViewFood.nameScoreNutri
      );
