...
$(function () {
  $.ajax({
    ...
    success: function (data) {
      ...
      $.map(data.ListNutritionals, function (val, key) {
        $("#listNutritionalsNotApply").append(
          '<option value="' + val.id + '">' + val.name + "</option>"
        );
      });
      $.map(data.ListWeights, function (val, key) {
        $("#listNutritionalsWeightType").append(
          '<option value="' + val.id + '">' + val.name + "</option>"
        );
      });
      $.map(data.ListWeights, function (val, key) {
        $("#listNutritionalsWeightTypeBase").append(
          '<option value="' + val.id + '">' + val.name + "</option>"
        );
      });
  ...
  $(".btn-warning").on("click", function () {
    ...
    foodNutritionalTable.clear().draw();
  ...
  $(".btnAddFoodNutritional").on("click", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          window.location.href = "./foodFoodList.php";
        }, 800);
      },
      error: function (data) {
  ...
  $.map(dataTableData, function (val, key) {
    if (val.elemState == elemState) {
      ...
      const line =
        ...
        '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddFoodNutritional"  onclick="ViewFoodNutritional(\'' +
        val.id +
        "', '" +
        val.nameNutritional +
        ...
        actionBtnDelete +
        '"  data-toggle="modal" data-target="#modalDel"                 onclick="DelFoodNutritional(\'' +
        val.id +
        "', '" +
        val.nameNutritional +
        ...
        '       <a class="dropdown-item ' +
        actionBtnRestore +
        '" data-toggle="modal" data-target="#modalRest"                onclick="RestFoodNutritional(\'' +
        val.id +
        "', '" +
        val.nameNutritional +
...
function ViewFoodNutritional(id, name) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".AddFoodNutritionalFormNutritionalWeightBase").val(
        data.FoodNutritional.weightTypeBase
      );
      ...
      $.map(data.ListNutritionals, function (val, key) {
        ...
        if (val.id == data.FoodNutritional.idNutritional) {
          ...
          $("#select2-listNutritionalsNotApply-container").html(
            val.id + " - " + val.name
          );
        }
      });
      ...
      $.map(data.ListWeights, function (val, key) {
        ...
        if (val.id == data.FoodNutritional.idWeightType) {
          ...
          $("#select2-listNutritionalsWeightType-container").html(
            val.id + " - " + val.name
          );
        }
        if (val.id == data.FoodNutritional.idWeightTypeBase) {
          $("#listNutritionalsWeightTypeBase").val(val.id);
          $("#select2-listNutritionalsWeightTypeBase-container").html(
            val.id + " - " + val.name
          );
        }
      });
