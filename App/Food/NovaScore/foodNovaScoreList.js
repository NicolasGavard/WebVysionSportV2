  ...
  if (name != "" || color != "") {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          window.location.href = "./foodNovaScoreList.php";
        }, 800);
      },
      error: function (data) {
  ...
  $.map(dataTableData, function (val, key) {
    if (val.elemState == elemState) {
      ...
      const line =
        ...
        '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddNovaScore" onclick="ViewNovaScore(\'' +
        val.id +
        '\');"                   href="#"><i class="dw dw-edit2"></i> Voir</a>' +
        '       <a class="dropdown-item ' +
        actionBtnDelete +
        '"  data-toggle="modal" data-target="#modalDel"         onclick="DelNovaScore(\'' +
        val.id +
        "', '" +
        val.number +
        ...
        '       <a class="dropdown-item ' +
        actionBtnRestore +
        '" data-toggle="modal" data-target="#modalRest"        onclick="RestNovaScore(\'' +
        val.id +
        "', '" +
        val.number +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".asColorPicker-trigger span").attr(
        "style",
        "background-color:" + data.ViewNovaScore.color
      );
