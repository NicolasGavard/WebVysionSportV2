  ...
  if (name != "") {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          window.location.href = "./foodLabelList.php";
        }, 800);
      },
      error: function (data) {
  ...
  $.map(dataTableData, function (val, key) {
    if (val.elemState == elemState) {
      ...
      const line =
        ...
        '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddLabel"   onclick="ViewLabel(\'' +
        val.id +
        '\');"                   href="#"><i class="dw dw-edit2"></i> Voir</a>' +
        '       <a class="dropdown-item ' +
        actionBtnDelete +
        '"  data-toggle="modal" data-target="#modalDel"        onclick="DelLabel(\'' +
        val.id +
        "', '" +
        val.name +
        ...
        '       <a class="dropdown-item ' +
        actionBtnRestore +
        '" data-toggle="modal" data-target="#modalRest"       onclick="RestLabel(\'' +
        val.id +
        "', '" +
        val.name +
