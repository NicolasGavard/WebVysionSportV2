...
//=============================================================================
  ...
  const SELECT = 'SELECT id,idusercoach,code,name,idexercisetype,linktopicture,size,type,description,elemstate,timestamp';
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "ExerciseStorData");
      }
    }
  ...
  public static function findByIdUserCoachCode(ExerciseStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdUserCoach(), 'index1'=>  $dataIn->getCode()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
  ...
  public static function update(ExerciseStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE exercise SET ";
  ...
  public static function save(ExerciseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(ExerciseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(ExerciseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(ExerciseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
    ...
    if ($inDbConnection != null) {
      ...
      $request .= "idusercoach,code,name,idexercisetype,linktopicture,size,type,description,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idusercoach,";
      $request .= ":code,";
      $request .= ":name,";
