...
$(function () {
  ...
  $("#ExerciseTypeTable tbody").on("click", "td", function () {
    viewDetail(this, exerciseTypeTable, "ViewExerciseType");
  });
  ...
  $(".AddNewExerciseType").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddExerciseTypeFormLanguageName" type="text" name="exerciseTypeLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
    ...
    if (code == "" || name == "") {
      ...
      $('input[name^="exerciseTypeLanguageName"]').each(function () {
        var idNameLanguage = this.name.substr(
          "exerciseTypeLanguageName".length
        );
        var idName = 0;
        var idExerciseType = 0;
        var timestampName = 0;
        var elemStateName = 0;
        if (exerciseTypeSelectedData != null) {
          $.map(
            exerciseTypeSelectedData.names,
      ...
      $.ajax({
        ...
        data: {
          ...
          elemState,
          timestamp,
          names: exerciseTypeNames,
        },
        success: function (data) {
          if (data.ConfirmSave) {
            $("#sa-success-distrix").trigger("click");
            setTimeout(function () {
              window.location.href = "./codeTableExerciseTypeList.php";
            }, 800);
          } else {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        line +=
          ...
          '       <a class="dropdown-item" data-toggle="modal" data-target="#modalAddExerciseType" id="ViewExerciseType' +
          val.id +
          '" onclick="ViewExerciseType(' +
          val.id +
          ');" href="#"><i class="dw dw-edit2"></i> Voir</a>' +
          '       <a class="dropdown-item ' +
          actionBtnDelete +
          '"  data-toggle="modal" data-target="#modalDel"         onclick="DelExerciseType(\'' +
          val.id +
          "', '" +
          val.name +
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"        onclick="RestExerciseType(\'' +
          val.id +
          "', '" +
          val.name +
    ...
    success: function (data) {
      ...
      $.map(languages, function (language, languageKey) {
        ...
        html +=
          ...
          className +
          ' AddExerciseTypeFormLanguageName" type="text" name="exerciseTypeLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
