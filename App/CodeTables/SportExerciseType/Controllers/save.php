...
if (isset($_POST)) {
  list($bodymember, $jsonError)       = DistriXCodeTableExerciseTypeData::getJsonData($_POST);
  list($bodymemberNames, $jsonError)  = DistriXCodeTableExerciseTypeNameData::getJsonArray($bodymember->getNames());
  ...
  $servicesCaller->setServiceName("App/CodeTables/SportExerciseType/Services/DistriXExerciseTypeSaveDataSvc.php");
  list($outputok, $output, $errorData) = $servicesCaller->call(); 
  // echo "-*/-"; print_r($output); echo "-*/-";

  $logOk = logController("Security_ExerciseType", "DistriXExerciseTypeSaveDataSvc", "SaveExerciseType", $output);
