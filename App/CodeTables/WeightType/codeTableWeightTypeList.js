...
$(function () {
  ...
  $(".AddNewWeightType").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddWeightTypeFormLanguageName" type="text" name="weightTypeLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.ConfirmSave) {
            ...
            setTimeout(function () {
              window.location.href = "./codeTableWeightTypeList.php";
            }, 800);
          } else {
  ...
  function ListWeightType(elemState) {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        if (val.isSolid == 1) {
          weightTypeType =
            '<div class="row"><div class="col-md-12 col-sm-12"><span class="micon dw dw-pyramid-chart"></span> ' +
            weightTypeType_solid +
            "</div>";
        }
        if (val.isLiquid == 1) {
          weightTypeType =
            '<div class="row"><div class="col-md-12 col-sm-12"><span class="micon dw dw-drop"></span> ' +
            weightTypeType_liquid +
            "</div>";
        }
        if (val.isOther == 1) {
          weightTypeType =
            '<div class="row"><div class="col-md-12 col-sm-12"><span class="micon dw dw-question"></span> ' +
            weightTypeType_other +
            "</div>";
        }
        ...
        line +=
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"          onclick="RestWeightType(\'' +
          val.id +
          "', '" +
          val.name +
    ...
    success: function (data) {
      ...
      $.map(languages, function (language, languageKey) {
        ...
        html +=
          ...
          className +
          ' AddWeightTypeFormLanguageName" type="text" name="weightTypeLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
