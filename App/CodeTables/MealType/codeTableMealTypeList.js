...
$(function () {
  ...
  $(".AddNewMealType").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddMealTypeFormLanguageName" type="text" name="mealTypeLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.ConfirmSave) {
            ...
            setTimeout(function () {
              window.location.href = "./codeTableMealTypeList.php";
            }, 800);
          } else {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        line +=
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"        onclick="RestMealType(\'' +
          val.id +
          "', '" +
          val.name +
    ...
    success: function (data) {
      ...
      $.map(languages, function (language, languageKey) {
        ...
        html +=
          ...
          className +
          ' AddMealTypeFormLanguageName" type="text" name="mealTypeLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
