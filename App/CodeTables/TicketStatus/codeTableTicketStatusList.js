  ...
  $("#btnAddTicketStatus, #btnUpdateTicketStatus").on('click', function() {
    ...
    if (code == "" || name == '') {
      ...
      $('input[name^="ticketStatusLanguageName"]').each(function() {
        var idNameLanguage = this.name.substr("ticketStatusLanguageName".length);
        var idName=0; var idTicketStatus=0; var timestampName=0; var elemStateName=0;
        if (ticketStatusSelectedData != null) {
          $.map(ticketStatusSelectedData.names, function(nameData, nameDataKey) {
      ...
      $.ajax({
        ...
        type : 'POST',
        dataType : 'JSON',
        data: {id,code,name,elemState,timestamp, "names":ticketStatusNames},
        success : function(data) {
          if (data.ConfirmSave) {
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      $.map(languages, function(language, languageKey) {
        ...
        html += '        <input class="form-control '+className+' AddTicketStatusFormLanguageName" type="text" name="ticketStatusLanguageName'+language.id+'" value="'+updateName+'" placeholder="'+nameTranslatedTxt+'">';
        html += '      </div>';
        html += '    </div>';
        html += '  </div>';
