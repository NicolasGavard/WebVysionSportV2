...
$(function () {
  ...
  $("#CategoryFoodTypeTable tbody").on("click", "td", function () {
    viewDetail(this, categoryFoodTypeTable, "ViewCategoryFoodType");
  });
  ...
  $(".btn-warning").on("click", function () {
    ...
    categoryFoodTypeTable.clear().draw();
  ...
  $(".AddNewCategoryFoodType").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddCategoryFoodTypeFormLanguageName" type="text" name="categoryFoodTypeLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
  ...
  $("#btnAddCategoryFoodType, #btnUpdateCategoryFoodType").on(
    "click",
    function () {
      ...
      if (code == "" || name == "") {
        if (code == "") {
          ...
          setTimeout(() => {
            $(".AddCategoryFoodTypeFormCode").removeClass(
              "form-control-danger"
            );
        ...
        if (name == "") {
          ...
          setTimeout(() => {
            $(".AddCategoryFoodTypeFormName").removeClass(
              "form-control-danger"
            );
        ...
        $('input[name^="categoryFoodTypeLanguageName"]').each(function () {
          var idNameLanguage = this.name.substr(
            "categoryFoodTypeLanguageName".length
          );
          var idName = 0;
          var idCategoryFoodType = 0;
          var timestampName = 0;
          var elemStateName = 0;
          if (categoryFoodTypeSelectedData != null) {
            $.map(
              categoryFoodTypeSelectedData.names,
        ...
        $.ajax({
          ...
          data: {
            ...
            elemState,
            timestamp,
            names: categoryFoodTypeNames,
          },
          success: function (data) {
            if (data.ConfirmSave) {
              $("#sa-success-distrix").trigger("click");
              setTimeout(function () {
                window.location.href = "./codeTableCategoryFoodTypeList.php";
              }, 800);
            } else {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        line +=
          ...
          '       <a class="dropdown-item" data-toggle="modal" data-target="#modalAddCategoryFoodType" id="ViewCategoryFoodType' +
          val.id +
          '"  onclick="ViewCategoryFoodType(' +
          val.id +
          ');" href="#"><i class="dw dw-edit2"></i> Voir</a>' +
          '       <a class="dropdown-item ' +
          actionBtnDelete +
          '"  data-toggle="modal" data-target="#modalDel"         onclick="DelCategoryFoodType(\'' +
          val.id +
          "', '" +
          val.name +
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"        onclick="RestCategoryFoodType(\'' +
          val.id +
          "', '" +
          val.name +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.map(languages, function (language, languageKey) {
        ...
        $.map(
          data.ViewCategoryFoodType.names,
          function (nameData, nameDataKey) {
        ...
        html +=
          ...
          className +
          ' AddCategoryFoodTypeFormLanguageName" type="text" name="categoryFoodTypeLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
