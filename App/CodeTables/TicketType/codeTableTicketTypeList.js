...
$(function () {
  ...
  $(".AddNewTicketType").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddTicketTypeFormLanguageName" type="text" name="ticketTypeLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.ConfirmSave) {
            ...
            setTimeout(function () {
              window.location.href = "./codeTableTicketTypeList.php";
            }, 800);
          } else {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        line +=
          ...
          '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddTicketType" onclick="ViewTicketType(\'' +
          val.id +
          '\');"                   href="#"><i class="dw dw-edit2"></i> Voir</a>' +
          '       <a class="dropdown-item ' +
          actionBtnDelete +
          '"  data-toggle="modal" data-target="#modalDel"         onclick="DelTicketType(\'' +
          val.id +
          "', '" +
          val.name +
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"        onclick="RestTicketType(\'' +
          val.id +
          "', '" +
          val.name +
    ...
    success: function (data) {
      ...
      $.map(languages, function (language, languageKey) {
        ...
        html +=
          ...
          className +
          ' AddTicketTypeFormLanguageName" type="text" name="ticketTypeLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
