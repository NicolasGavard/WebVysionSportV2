...
$(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.map(data.ListBodyMembers, function (val, key) {
        $("#listBodyMuscleMembers").append(
          '<option value="' + val.id + '">' + val.names[0].name + "</option>"
        );
      });
  ...
  $(".AddNewBodyMuscle").on("click", function () {
    ...
    $.map(languages, function (language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html +=
        '        <input class="form-control AddBodyMuscleFormLanguageName" type="text" name="bodyMuscleLanguageName' +
        language.id +
        '" placeholder="' +
        nameTranslatedTxt +
        '">';
    ...
    if (noError) {
      ...
      $.ajax({
        ...
        data: {
          ...
          elemState,
          timestamp,
          names: bodyMuscleNames,
        },
        success: function (data) {
          if (data.ConfirmSave) {
            $("#sa-success-distrix").trigger("click");
            setTimeout(function () {
              window.location.href = "./codeTableBodyMuscleList.php";
            }, 800);
          } else {
    ...
    $.map(dataTableData, function (val, key) {
      if (val.elemState == elemState) {
        ...
        line +=
          ...
          '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddBodyMuscle" onclick="ViewBodyMuscle(\'' +
          val.id +
          '\');"                   href="#"><i class="dw dw-edit2"></i> Voir</a>' +
          '       <a class="dropdown-item ' +
          actionBtnDelete +
          '"  data-toggle="modal" data-target="#modalDel"         onclick="DelBodyMuscle(\'' +
          val.id +
          "', '" +
          val.name +
          ...
          '       <a class="dropdown-item ' +
          actionBtnRestore +
          '" data-toggle="modal" data-target="#modalRest"        onclick="RestBodyMuscle(\'' +
          val.id +
          "', '" +
          val.name +
...
function ViewBodyMuscle(id) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#listBodyMuscleMembers")
        .val(data.ViewBodyMuscle.idBodyMember)
        .change();
      $("#bodyMuscleLanguages").html("");
      ...
      $.map(languages, function (language, languageKey) {
        ...
        html +=
          ...
          className +
          ' AddBodyMuscleFormLanguageName" type="text" name="bodyMuscleLanguageName' +
          language.id +
          '" value="' +
          updateName +
          '" placeholder="' +
