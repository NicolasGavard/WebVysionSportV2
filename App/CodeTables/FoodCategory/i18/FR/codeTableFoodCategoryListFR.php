<?php
  $page_title           = "Catégories d'aliments";
  $page_code            = "Code";
  $page_name            = "Nom";
  $page_translation     = "Traductions";
  $page_languages       = "Langues";
  $page_language        = "Langue";
  $page_name_translated = "Nom dans la langue";
  $page_status          = "Statut";
  $page_action          = "Actions";
  $page_add_title       = "Ajouter un catégorie d'aliment";
  $page_update_title    = "Modifier le catégorie d'aliment";
  $page_delete_title    = "Supprimer le catégorie d'aliment";
  $page_restore_title   = "Restaurer le catégorie d'aliment";

  $errorData_txt_code   = "Le code du catégorie d'aliment est manquant";
  $errorData_txt_name   = "Le nom du catégorie d'aliment est manquant";
  $errorData_txt_langue = "La langue du catégorie d'aliment est manquante";

  $error_400_txt = "Le Code {0} est déjà utilisé";
  $error_401_txt = "Les informations de {0} ont été modifiées par un autre utilisateur. Merci d'afficher à nouveau la page pour voir les modifications.";
  $error_402_txt = "Les informations de langues de {0} ont été modifiées par un autre utilisateur. Merci d'afficher à nouveau la page pour voir les modifications.";
