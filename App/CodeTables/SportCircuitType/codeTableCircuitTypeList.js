...
$(function() {
  ...
  $(".AddNewCircuitType").on('click', function() {
    ...
    $.map(languages, function(language, languageKey) {
      ...
      html += '      <div class="form-group">';
      html += '        <input class="form-control AddCircuitTypeFormLanguageName" type="text" name="circuitTypeLanguageName'+language.id+'" placeholder="'+nameTranslatedTxt+'">';
      html += '      </div>';
      html += '    </div>';
      html += '  </div>';
  ...
  $("#btnAddCircuitType, #btnUpdateCircuitType").on('click', function() {
    ...
    if (code == "" || name == '') {
      ...
      $('input[name^="circuitTypeLanguageName"]').each(function() {
        ...
        var idName=0; var idCircuitType=0; var timestampName=0; var elemStateName=0;
        if (circuitTypeSelectedData != null) {
          $.map(circuitTypeSelectedData.names, function(nameData, nameDataKey) {
      ...
      $.ajax({
        ...
        success : function(data) {
          if (data.ConfirmSave) {
            ...
            setTimeout(function() {window.location.href = "./codeTableCircuitTypeList.php";}, 800);        
          } else {
    ...
    $.map(dataTableData, function(val, key) {
      if(val.elemState == elemState){
        ...
        line += '</td>'+
                ...
                '       <a class="dropdown-item"                      data-toggle="modal" data-target="#modalAddCircuitType" onclick="ViewCircuitType(\''+val.id+'\');"                   href="#"><i class="dw dw-edit2"></i> Voir</a>'+
                '       <a class="dropdown-item '+actionBtnDelete+'"  data-toggle="modal" data-target="#modalDel"         onclick="DelCircuitType(\''+val.id+'\', \''+val.name+'\');"  href="#"><i class="dw dw-delete-3"></i> Supprimer</a>'+
                '       <a class="dropdown-item '+actionBtnRestore+'" data-toggle="modal" data-target="#modalRest"        onclick="RestCircuitType(\''+val.id+'\', \''+val.name+'\');" href="#"><i class="dw dw-share-2"></i> Restaurer</a>'+
                '     </div>'+
                '   </div>'+
                ' </td>'+
                '</tr>';
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      $.map(languages, function(language, languageKey) {
        ...
        html += '        <input class="form-control '+className+' AddCircuitTypeFormLanguageName" type="text" name="circuitTypeLanguageName'+language.id+'" value="'+updateName+'" placeholder="'+nameTranslatedTxt+'">';
        html += '      </div>';
        html += '    </div>';
        html += '  </div>';
