...
class EnterpriseStor
{
  public static function getParentList(bool $all, DistriXPDOConnection $inDbConnection): array
  {
    $request = "";
    $data = new EnterpriseStorData();
    $list = array();
    $listInd = 0;

    if ($inDbConnection != null) {
      $request  = "SELECT entc.id id,entc.name name,entc.city city,entc.statut statut,entp.id identerpriseparent";
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "EnterpriseStorData");
      }
    }
  ...
  const SELECT = 'SELECT id,code,name,email,phone,mobile,co,street,zipcode,city,logoimage,logoimagehtmlname,logoimagename,logosize,logotype,idregion,idcountry,idlanguage,idusermanager,identerpriseparent,statut,timestamp';
  const FROM = ' FROM enterprise';
  ...
  public static function findByCode(EnterpriseStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new EnterpriseStorData();
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "EnterpriseStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  public static function findByName(EnterpriseStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function update(EnterpriseStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
    ...
    if ($inDbConnection != null) {
      ...
      $request .= "identerpriseparent= :identerpriseparent,";
      ...
      $params["logoimage"] = $data->getLogoImage();
      ...
      $params["identerpriseparent"] = $data->getIdEnterpriseParent();
  ...
  public static function save(EnterpriseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $id = 0;
    if ($data->getId() > 0) {
  ...
  public static function remove(EnterpriseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(EnterpriseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(EnterpriseStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
    ...
    if ($inDbConnection != null) {
      ...
      $request .= "code,name,email,phone,mobile,co,street,zipcode,city,logoimage,logoimagehtmlname,logoimagename,logosize,logotype,idregion,idcountry,idlanguage,idusermanager,identerpriseparent,statut,timestamp)";
      $request .= " VALUES(";
      $request .= ":code,";
      $request .= ":name,";
      $request .= ":email,";
