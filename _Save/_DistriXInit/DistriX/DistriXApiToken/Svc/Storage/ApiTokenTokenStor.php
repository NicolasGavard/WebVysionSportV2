...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "apitokentoken";
  const SELECT = 'SELECT id,idapitokenclient,token,tokendate,tokentime,keyused,tokennbuse,timestamp';
  const FROM = ' FROM apitokentoken';
  ...
  public static function getList(DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "ApiTokenTokenStorData");
      }
    }
  ...
  public static function findByIdApiTokenClient(ApiTokenTokenStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new ApiTokenTokenStorData();

    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idapitokenclient = :index0";
      $request .= " AND token = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdApiTokenClient(), 'index1'=>  $dataIn->getToken()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "ApiTokenTokenStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of findByIdApiTokenClient
  ...
  public static function update(ApiTokenTokenStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE apitokentoken SET ";
      $request .= "idapitokenclient= :idapitokenclient,";
      $request .= "token= :token,";
      $request .= "tokendate= :tokendate,";
      $request .= "tokentime= :tokentime,";
      $request .= "keyused= :keyused,";
      $request .= "tokennbuse= :tokennbuse,";
      $request .= "timestamp= :timestamp";
      $request .= " WHERE id = :id";
      $request .= " AND timestamp = :oldtimestamp";
      $params = [];
      $params["id"] = $data->getId();
      $params["idapitokenclient"] = $data->getIdApiTokenClient();
      $params["token"] = $data->getToken();
      $params["tokendate"] = $data->getTokenDate();
      $params["tokentime"] = $data->getTokenTime();
      $params["keyused"] = $data->getKeyUsed();
      $params["tokennbuse"] = $data->getTokenNbUse();
  ...
  public static function save(ApiTokenTokenStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM apitokentoken";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(ApiTokenTokenStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO apitokentoken(";
      $request .= "idapitokenclient,token,tokendate,tokentime,keyused,tokennbuse,timestamp)";
      $request .= " VALUES(";
      $request .= ":idapitokenclient,";
      $request .= ":token,";
      $request .= ":tokendate,";
      $request .= ":tokentime,";
      $request .= ":keyused,";
