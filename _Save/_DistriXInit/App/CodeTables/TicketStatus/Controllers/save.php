...
if (isset($_POST)) {
  ...
  $ticketStatus->setNames([]); // Needed to be sent without an array fulfilled with elements that are not data objects. 01 June 22
  ...
  $servicesCaller->setServiceName("App/CodeTables/TicketStatus/Services/DistriXTicketStatusSaveDataSvc.php");
  ...
  if ($outputok && !empty($output) > 0 && isset($output["ConfirmSave"])) {
    ...
    if (isset($$errorCode)) {
      $codes[0] = $ticketStatus->getCode();
      $codes[1] = $ticketStatus->getId();
