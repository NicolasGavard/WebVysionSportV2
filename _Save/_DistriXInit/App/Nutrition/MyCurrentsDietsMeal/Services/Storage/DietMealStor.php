...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "dietmeal";
  const SELECT = 'SELECT id,iddiet,iddietrecipe,daynumber,idmealtype,elemstate,timestamp';
  const FROM = ' FROM dietmeal';
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "DietMealStorData");
      }
    }
  ...
  public static function findByIdDietIdDietRecipeDayNumberIdMealType(DietMealStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE iddiet = :index0";
      $request .= " AND iddietrecipe = :index1";
      $request .= " AND daynumber = :index2";
      $request .= " AND idmealtype = :index3";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdDiet(), 'index1'=>  $dataIn->getIdDietRecipe(), 'index2'=>  $dataIn->getDayNumber(), 'index3'=>  $dataIn->getIdMealType()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "DietMealStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of IddietIdDietRecipeDayNumberIdMealType

  public static function findByIdDiet(DietMealStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      $params = [];
      $params["index0"] = $dataIn->getIdDiet();
      if (!$all) {
  ...
  public static function findByIdDietRecipe(DietMealStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE iddietrecipe = :index0";
      if (!$all) {
        $request .= " AND elemstate = :statut";
      }
      $params = [];
      $params["index0"] = $dataIn->getIdDietRecipe();
      if (!$all) {
        $params["statut"] = $dataIn->getAvailableValue();
      }
      $stmt = $inDbConnection->prepare($request);
  ...
  public static function findByIdMealType(DietMealStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idmealtype = :index0";
      if (!$all) {
        $request .= " AND elemstate = :statut";
      }
      $params = [];
      $params["index0"] = $dataIn->getIdMealType();
      if (!$all) {
        $params["statut"] = $dataIn->getAvailableValue();
      }
      $stmt = $inDbConnection->prepare($request);
  ...
  public static function update(DietMealStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE dietmeal SET ";
      $request .= "iddiet= :iddiet,";
      $request .= "iddietrecipe= :iddietrecipe,";
      ...
      $params["iddiet"] = $data->getIdDiet();
      $params["iddietrecipe"] = $data->getIdDietRecipe();
      $params["daynumber"] = $data->getDayNumber();
  ...
  public static function save(DietMealStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(DietMealStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(DietMealStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(DietMealStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO dietmeal(";
      $request .= "iddiet,iddietrecipe,daynumber,idmealtype,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":iddiet,";
      $request .= ":iddietrecipe,";
      $request .= ":daynumber,";
      $request .= ":idmealtype,";
      $request .= ":elemstate,";
      $request .= ":timestamp)";
