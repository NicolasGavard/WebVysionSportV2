...
if ($dataSvc->isAuthorized()) {
  ...
  if (is_null($dbConnection->getError())) {
    if ($dbConnection->beginTransaction()) {
      ...
      $insere = RecipeFoodStor::restore($recipeFoodStorData, $dbConnection);
      if ($insere) {
        $dbConnection->commit();
      } else {
  ...
  if ($errorData != null) {
    $errorData->setApplicationModuleFunctionalityCodeAndFilename("DistrixSty", "RestoreCurrentRecipeFood", $dataSvc->getMethodName(), basename(__FILE__));
