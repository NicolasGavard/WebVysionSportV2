...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "ticketpicture";
  const SELECT = 'SELECT id,idticket,idticketcomment,linktopicture,size,type,date,time,elemstate,timestamp';
  const FROM = ' FROM ticketpicture';
  ...
  {
    $request = "";
    $data = new TicketPictureStorData();
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketPictureStorData");
      }
    }
  ...
  public static function getListFromList(array $inList, bool $all, string $className, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null && (!is_null($inList)) && (!empty($inList))) {
      if ($className == "" || is_null($className)) {
        $className = "TicketPictureStorData";
      }
      $request  = self::SELECT;
      $request .= self::FROM;
  ...
  public static function findByIdTicketIdTicketCommentDateTime(TicketPictureStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new TicketPictureStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idticket = :index0";
      $request .= " AND idticketcomment = :index1";
      $request .= " AND date = :index2";
      $request .= " AND time = :index3";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdTicket(), 'index1'=>  $dataIn->getIdTicketComment(), 'index2'=>  $dataIn->getDate(), 'index3'=>  $dataIn->getTime()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketPictureStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of IdTicketIdTicketCommentDateTime

  public static function findByIdTicket(TicketPictureStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      $params = [];
      $params["index0"] = $dataIn->getIdTicket();
      if (!$all) {
  ...
  public static function findByIdTicketComment(TicketPictureStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idticketcomment = :index0";
      if (!$all) {
        $request .= " AND elemstate = :statut";
      }
      $params = [];
      $params["index0"] = $dataIn->getIdTicketComment();
      if (!$all) {
        $params["statut"] = $dataIn->getAvailableValue();
      }
      $stmt = $inDbConnection->prepare($request);
  ...
  public static function update(TicketPictureStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE ticketpicture SET ";
      $request .= "idticket= :idticket,";
      $request .= "idticketcomment= :idticketcomment,";
      ...
      $params["idticket"] = $data->getIdTicket();
      $params["idticketcomment"] = $data->getIdTicketComment();
      $params["linktopicture"] = $data->getLinkToPicture();
      $params["size"] = $data->getSize();
      $params["type"] = $data->getType();
      $params["date"] = $data->getDate();
      $params["time"] = $data->getTime();
  ...
  public static function save(TicketPictureStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(TicketPictureStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(TicketPictureStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM ticketpicture";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(TicketPictureStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO ticketpicture(";
      $request .= "idticket,idticketcomment,linktopicture,size,type,date,time,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idticket,";
      $request .= ":idticketcomment,";
