...
//=============================================================================
  ...
  const SELECT = 'SELECT id,idticket,iduser,title,descmessage,date,time,elemstate,timestamp';
  const FROM = ' FROM ticketcomment';
  ...
  public static function findByIdTicketIdUserDateTime(TicketCommentStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdTicket(), 'index1'=>  $dataIn->getIdUser(), 'index2'=>  $dataIn->getDate(), 'index3'=>  $dataIn->getTime()]);
  ...
  public static function update(TicketCommentStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "UPDATE ticketcomment SET ";
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM ticketcomment";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(TicketCommentStorData $data, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "INSERT INTO ticketcomment(";
      $request .= "idticket,iduser,title,descmessage,date,time,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idticket,";
      $request .= ":iduser,";
      $request .= ":title,";
