...
//=============================================================================
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketStorData");
      }
    }
  ...
  public static function findByIdUserCreateIdUserAssignDateTime(TicketStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new TicketStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idusercreate = :index0";
      $request .= " AND iduserassign = :index1";
      $request .= " AND date = :index2";
      $request .= " AND time = :index3";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdUserCreate(), 'index1'=>  $dataIn->getIdUserAssign(), 'index2'=>  $dataIn->getDate(), 'index3'=>  $dataIn->getTime()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of IdUserCreateIdUserAssignDateTime

  public static function findByIdUser(TicketStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      $request .= self::FROM;
      $request .= " WHERE idusercreate = :index0";
      $request .= " OR iduserassign = :index1";
      if (!$all) {
        $request .= " AND elemstate = :statut";
      }
      $params = [];
      $params["index0"] = $dataIn->getIdUserCreate();
      $params["index1"] = $dataIn->getIdUserAssign();
      if (!$all) {
  ...
  public static function findByIdUserCreate(TicketStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function findByIdUserAssign(TicketStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE iduserassign = :index0";
      if (!$all) {
        $request .= " AND elemstate = :statut";
      }
      $params = [];
      $params["index0"] = $dataIn->getIdUserAssign();
      if (!$all) {
        $params["statut"] = $dataIn->getAvailableValue();
      }
      $stmt = $inDbConnection->prepare($request);
  ...
  public static function update(TicketStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
  ...
  public static function save(TicketStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(TicketStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(TicketStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(TicketStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
