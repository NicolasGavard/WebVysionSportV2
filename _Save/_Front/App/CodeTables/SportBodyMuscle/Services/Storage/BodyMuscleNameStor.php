...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "bodymusclename";
  const SELECT = 'SELECT id,idbodymuscle,idlanguage,name,elemstate,timestamp';
  const FROM = ' FROM bodymusclename';
  ...
  {
    $request = "";
    $data = new BodyMuscleNameStorData();
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "BodyMuscleNameStorData");
      }
    }
  ...
  public static function getListFromList(array $inList, bool $all, string $className, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null && (!is_null($inList)) && (!empty($inList))) {
      if ($className == "" || is_null($className)) {
        $className = "BodyMuscleNameStorData";
      }
      $request  = self::SELECT;
      $request .= self::FROM;
  ...
  public static function findByindbodyMuscleunique(BodyMuscleNameStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new BodyMuscleNameStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idbodymuscle = :index0";
      $request .= " AND idlanguage = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdBodyMuscle(), 'index1'=>  $dataIn->getIdLanguage()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "BodyMuscleNameStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of indbodyMuscleunique

  public static function findByindlanguage(BodyMuscleNameStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function update(BodyMuscleNameStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE bodymusclename SET ";
      $request .= "idbodymuscle= :idbodymuscle,";
      ...
      $params["idbodymuscle"] = $data->getIdBodyMuscle();
  ...
  public static function save(BodyMuscleNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(BodyMuscleNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(BodyMuscleNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM bodymusclename";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(BodyMuscleNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO bodymusclename(";
      $request .= "idbodymuscle,idlanguage,name,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idbodymuscle,";
      $request .= ":idlanguage,";
      $request .= ":name,";
      $request .= ":elemstate,";
      $request .= ":timestamp)";
