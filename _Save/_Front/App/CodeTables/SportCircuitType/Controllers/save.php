...
if (isset($_POST)) {
  list($bodymember, $jsonError)       = DistriXCodeTableCircuitTypeData::getJsonData($_POST);
  list($bodymemberNames, $jsonError)  = DistriXCodeTableCircuitTypeNameData::getJsonArray($bodymember->getNames());
  ...
  $servicesCaller->setServiceName("App/CodeTables/SportCircuitType/Services/DistriXCircuitTypeSaveDataSvc.php");
  list($outputok, $output, $errorData) = $servicesCaller->call(); 
  // echo "-*/-"; print_r($output); echo "-*/-";

  $logOk = logController("Security_CircuitType", "DistriXCircuitTypeSaveDataSvc", "SaveCircuitType", $output);
