...
if (isset($_POST)) {
  list($categoryFoodType, $jsonError) = DistriXCodeTableCategoryFoodTypeData::getJsonData($_POST);
  list($categoryFoodTypeNames, $jsonError) = DistriXCodeTableCategoryFoodTypeNameData::getJsonArray($categoryFoodType->getNames());
  $categoryFoodType->setNames([]); // Needed to be sent without an array fulfilled with elements that are not data objects. 01 June 22
  ...
  $servicesCaller->addParameter("dataNames", $categoryFoodTypeNames);
  $servicesCaller->setServiceName("App/CodeTables/CategoryFoodType/Services/DistriXCategoryFoodTypeSaveDataSvc.php");
  ...
  if ($outputok && !empty($output) > 0 && isset($output["ConfirmSave"])) {
    ...
    if (isset($$errorCode)) {
      $codes[0] = $categoryFoodType->getCode();
      $codes[1] = $categoryFoodType->getId();
