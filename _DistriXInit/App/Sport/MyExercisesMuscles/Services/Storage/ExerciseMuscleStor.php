...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "exercisemuscle";
  const SELECT = 'SELECT id,idexercise,idbodymuscle,elemstate,timestamp';
  const FROM = ' FROM exercisemuscle';
  ...
  public static function findByIdExerciseIdBodyMuscle(ExerciseMuscleStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      $request .= self::FROM;
      $request .= " WHERE idexercise = :index0";
      $request .= " AND idbodymuscle = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdExercise(), 'index1'=>  $dataIn->getIdBodyMuscle()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "ExerciseMuscleStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  public static function update(ExerciseMuscleStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE exercisemuscle SET ";
  ...
  public static function save(ExerciseMuscleStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(ExerciseMuscleStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(ExerciseMuscleStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM exercisemuscle";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(ExerciseMuscleStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO exercisemuscle(";
      $request .= "idexercise,idbodymuscle,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idexercise,";
