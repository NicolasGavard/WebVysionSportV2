  ...
  $.ajax({
    ...
    success : function(data) {
      $.map(data.ListMyCurrentsDiets, function(val, key) {
        ...
        if (nbStudent < 2) {
          spanListUserAssigned = '<span class="page_nutrition_my_template_diet_list_assigned_for_one"> Elève</span>';
        } else {
          spanListUserAssigned = '<span class="page_nutrition_my_template_diet_list_assigned_for_plur"> Elèves</span>';
        }
...
function ViewMyCurrentsDiets(id){
  $.ajax({
    ...
    success : function(data) {
      ...
      $(".avatar-my_template_diet").attr("src", data.ViewMyCurrentsDiets.linkToPicture);
