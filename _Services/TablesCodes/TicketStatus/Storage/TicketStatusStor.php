...
class TicketStatusStor {
  const TABLE_NAME_TABLENAME = "ticketstatusname";
  const FIELDS_TABLENAME = ',ticketstatusname.id ticketstatusnameid,idticketstatus ticketstatusnameidticketstatus,idlanguage ticketstatusnameidlanguage,ticketstatusname.name ticketstatusname,ticketstatusname.elemstate ticketstatusnameelemstate,ticketstatusname.timestamp ticketstatusnametimestamp';

  public static function getListNames(bool $all, TicketStatusNameStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new TicketStatusStorData();
    $list = [];
    $listNames = [];
  
    if ($inDbConnection != null) {
      $request  = "SELECT ticketstatus.id,ticketstatus.code,ticketstatus.name,ticketstatus.elemstate,ticketstatus.timestamp";
      $request .= self::FIELDS_TABLENAME;
      $request .= self::FROM;
      $request .= " LEFT JOIN ".self::TABLE_NAME_TABLENAME." ON ".self::TABLE_NAME.".id = ".self::TABLE_NAME_TABLENAME.".idticketstatus";
      ...
      if ($stmt->rowCount() > 0) {
        ...
        while (($row = $stmt->fetch(PDO::FETCH_ASSOC)) !== false) {
          ...
          if (! is_null($row["ticketstatusnameid"])) {
            $dataName->setId($row["ticketstatusnameid"]);
            $dataName->setIdTicketStatus($row["ticketstatusnameidticketstatus"]);
            $dataName->setIdLanguage($row["ticketstatusnameidlanguage"]);
            $dataName->setName($row["ticketstatusname"]);
            $dataName->setElemState($row["ticketstatusnameelemstate"]);
            $dataName->setTimestamp($row["ticketstatusnametimestamp"]);
          }
  ...
  public static function findByIndCodeNames(TicketStatusStorData $dataIn, TicketStatusNameStorData $dataNameIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new TicketStatusStorData();
    $listNames = [];
    ...
    if ($inDbConnection != null) {
      ...
      $request .= " WHERE ticketstatus.code = :index0";
      $stmt = $inDbConnection->prepare($request);
...
//=============================================================================
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketStatusStorData");
      }
    }
  ...
  public static function getListFromList(array $inList, bool $all, string $className, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null && (!is_null($inList)) && (!empty($inList))) {
      if ($className == "" || is_null($className)) {
        $className = "TicketStatusStorData";
      }
      $request  = self::SELECT;
      $request .= self::FROM;
  ...
  public static function findByCode(TicketStatusStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new TicketStatusStorData();
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "TicketStatusStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  public static function update(TicketStatusStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE ticketstatus SET ";
      $request .= "code= :code,";
      $request .= "name= :name,";
  ...
  public static function save(TicketStatusStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(TicketStatusStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(TicketStatusStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM ticketstatus";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(TicketStatusStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO ticketstatus(";
