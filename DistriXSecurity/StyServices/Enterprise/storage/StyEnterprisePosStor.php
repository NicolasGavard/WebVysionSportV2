...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "enterprisepos";
  const SELECT = 'SELECT id,styidenterprise,idpos,statut,timestamp';
  const FROM = ' FROM styenterprisepos';
  ...
  {
    $request = "";
    $data = new StyEnterprisePosStorData();
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      $request .= " ORDER BY styidenterprise";
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "StyEnterprisePosStorData");
      }
    }
  ...
  public static function findByEnterprisePos(StyEnterprisePosStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new StyEnterprisePosStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE styidenterprise = :index0";
      $request .= " AND idpos = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getStyIdEnterprise(), 'index1'=>  $dataIn->getIdPos()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "StyEnterprisePosStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of findByEnterprisePos

  public static function findByPosEnterprise(StyEnterprisePosStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new StyEnterprisePosStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idpos = :index0";
      $request .= " AND styidenterprise = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdPos(), 'index1'=>  $dataIn->getStyIdEnterprise()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
  ...
  public static function update(StyEnterprisePosStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE enterprisepos SET ";
      $request .= "styidenterprise= :styidenterprise,";
      $request .= "idpos= :idpos,";
      ...
      $params["styidenterprise"] = $data->getStyIdEnterprise();
      $params["idpos"] = $data->getIdPos();
  ...
  public static function save(StyEnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(StyEnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(StyEnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM enterprisepos";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(StyEnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO enterprisepos(";
      $request .= "styidenterprise,idpos,statut,timestamp)";
      $request .= " VALUES(";
      $request .= ":styidenterprise,";
      $request .= ":idpos,";
      $request .= ":statut,";
      $request .= ":timestamp)";
      $params = [];
