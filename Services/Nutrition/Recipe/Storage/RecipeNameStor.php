...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "recipename";
  const SELECT = 'SELECT id,idrecipe,idlanguage,name,statut,timestamp';
  const FROM = ' FROM recipename';
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "RecipeNameStorData");
      }
    }
  ...
  public static function findByIdRecipe(RecipeNameStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function findByIdLanguageIdRecipe(RecipeNameStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function read(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "RecipeNameStorData");
        $data = $stmt->fetch();
      }
  ...
  public static function update(RecipeNameStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE recipename SET ";
  ...
  public static function save(RecipeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(RecipeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(RecipeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM recipename";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(RecipeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO recipename(";
      $request .= "idrecipe,idlanguage,name,statut,timestamp)";
      $request .= " VALUES(";
      $request .= ":idrecipe,";
      $request .= ":idlanguage,";
      $request .= ":name,";
