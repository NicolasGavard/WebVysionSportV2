...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "foodlabel";
  const SELECT = 'SELECT id,idfood,idlabel,elemstate,timestamp';
  const FROM = ' FROM foodlabel';
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "FoodLabelStorData");
      }
    }
  ...
  public static function findByidFoodIdLabel(FoodLabelStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new FoodLabelStorData();
    ...
    if ($inDbConnection != null) {
      ...
      $request .= " AND idlabel = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdFood(), 'index1'=>  $dataIn->getIdLabel()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "FoodLabelStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of idFoodIdLabel

  public static function findByIdFood(FoodLabelStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function findByIdLabel(FoodLabelStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function update(FoodLabelStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE foodlabel SET ";
      $request .= "idfood= :idfood,";
      $request .= "idlabel= :idlabel,";
  ...
  public static function save(FoodLabelStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(FoodLabelStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(FoodLabelStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(FoodLabelStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO foodlabel(";
      $request .= "idfood,idlabel,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idfood,";
      $request .= ":idlabel,";
      $request .= ":elemstate,";
