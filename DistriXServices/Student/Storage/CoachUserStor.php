...
//=============================================================================
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "CoachUserStorData");
      }
    }
  ...
  public static function findByStyIdUserCoachStyIdUserStatut(CoachUserStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "CoachUserStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  // End of IndStyIdUserCoach

  public static function findByStyIdUser(CoachUserStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function update(CoachUserStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE coachuser SET ";
      $request .= "styIdUserCoach= :styIdUserCoach,";
  ...
  public static function save(CoachUserStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(CoachUserStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(CoachUserStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(CoachUserStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
