...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "foodtypename";
  const SELECT = 'SELECT id,idfoodtype,idlanguage,name,elemstate,timestamp';
  const FROM = ' FROM foodtypename';
  ...
  public static function getList(bool $all, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "FoodTypeNameStorData");
      }
    }
  ...
  public static function getListFromList(array $inList, bool $all, string $className, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null && (!is_null($inList)) && (!empty($inList))) {
      if ($className == "" || is_null($className)) {
        $className = "FoodTypeNameStorData";
      }
      $request  = self::SELECT;
      $request .= self::FROM;
  ...
  public static function findByindfoodTypeunique(FoodTypeNameStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new FoodTypeNameStorData();
    ...
    if ($inDbConnection != null) {
      $request  = self::SELECT;
      $request .= self::FROM;
      $request .= " WHERE idfoodtype = :index0";
      $request .= " AND idlanguage = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdFoodType(), 'index1'=>  $dataIn->getIdLanguage()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "FoodTypeNameStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  }
  // End of indfoodTypeunique

  public static function findByindlanguage(FoodTypeNameStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
  ...
  public static function update(FoodTypeNameStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE foodtypename SET ";
      $request .= "idfoodtype= :idfoodtype,";
      ...
      $params["idfoodtype"] = $data->getIdFoodType();
  ...
  public static function save(FoodTypeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(FoodTypeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(FoodTypeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM foodtypename";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(FoodTypeNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO foodtypename(";
      $request .= "idfoodtype,idlanguage,name,elemstate,timestamp)";
      $request .= " VALUES(";
      $request .= ":idfoodtype,";
      $request .= ":idlanguage,";
      $request .= ":name,";
