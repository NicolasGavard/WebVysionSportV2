...
//=============================================================================
//=============================================================================
  const TABLE_NAME = "nutritionalname";
  const SELECT = 'SELECT id,idnutritional,idlanguage,name,elemstate,timestamp';
  const FROM = ' FROM nutritionalname';
  ...
  public static function findByNutritionalIdLanguage(NutritionalNameStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      ...
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdNutritional(), 'index1'=>  $dataIn->getIdLanguage()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "NutritionalNameStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  public static function update(NutritionalNameStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "UPDATE nutritionalname SET ";
  ...
  public static function save(NutritionalNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(NutritionalNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(NutritionalNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function delete(int $id, DistriXPDOConnection $inDbConnection)
  {
    ...
    if ($inDbConnection != null) {
      $request  = "DELETE FROM nutritionalname";
      $request .= " WHERE id = :id";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['id'=> $id]);
      $insere = is_null($inDbConnection->errorInfo()[2]); // If there is no DB error
  ...
  public static function create(NutritionalNameStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";

    if ($inDbConnection != null) {
      $request  = "INSERT INTO nutritionalname(";
      $request .= "idnutritional,idlanguage,name,elemstate,timestamp)";
      $request .= " VALUES(";
