...
//=============================================================================
  ...
  const SELECT = 'SELECT id,identerprise,idpos,statut,timestamp';
  const FROM = ' FROM enterprisepos';
  ...
  {
    $request = "";
    $data = new EnterprisePosStorData();
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      if ($stmt->rowCount() > 0) {
        $list = $stmt->fetchAll(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "EnterprisePosStorData");
      }
    }
  ...
  public static function findByEnterprisePos(EnterprisePosStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new EnterprisePosStorData();
    ...
    if ($inDbConnection != null) {
      ...
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdEnterprise(), 'index1'=>  $dataIn->getIdPos()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
        $stmt->setFetchMode(PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "EnterprisePosStorData");
        $data = $stmt->fetch();
      }
    }
    return $data;
  ...
  public static function findByPosEnterprise(EnterprisePosStorData $dataIn, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $data = new EnterprisePosStorData();
    ...
    if ($inDbConnection != null) {
      ...
      $request .= " AND identerprise = :index1";
      $stmt = $inDbConnection->prepare($request);
      $stmt->execute(['index0'=>  $dataIn->getIdPos(), 'index1'=>  $dataIn->getIdEnterprise()]);
      if (self::SHOW_FIND_REQUEST) {
        echo self::DEBUG_ERROR . $inDbConnection->errorInfo()[2] . self::BREAK . $stmt->debugDumpParams() . self::DOUBLE_BREAK;
      }
      if ($stmt->rowCount() > 0) {
  ...
  public static function update(EnterprisePosStorData $data, $traceType, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
  ...
  public static function save(EnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false; $id = 0;
    if ($data->getId() > 0) {
      $id = $data->getId();
  ...
  public static function remove(EnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
  ...
  public static function restore(EnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    if ($data->getId() > 0) {
      $data = self::read($data->getId(), $inDbConnection);
  ...
  public static function create(EnterprisePosStorData $data, DistriXPDOConnection $inDbConnection)
  {
    $insere = false;
    $request = "";
    ...
    if ($inDbConnection != null) {
      ...
      $request .= "identerprise,idpos,statut,timestamp)";
      $request .= " VALUES(";
