...
class StyEnterpriseStor
{
  ...
  public static function findByIdParent(StyEnterpriseStorData $dataIn, bool $all, DistriXPDOConnection $inDbConnection)
  {
    $request = "";
    $list = [];
    ...
    if ($inDbConnection != null) {
      ...
      $params = [];
      $params["index0"] = $dataIn->getIdStyEnterpriseParent();
      if (!$all) {
        $params["statut"] = $dataIn->getStatus();
      }
      $stmt = $inDbConnection->prepare($request);
